...
public interface IScoreDAO {
    ...
    @Insert("insert into beihua.score (`course_id`,`student_id`,`grade`,`grade_point`,`access_credits`,`is_rework`,`fail_reason`,`rework_situation`,`addtime`,`modtime`)" +
            " values (#{courseId},#{studentId},#{gradePoint},#{accessCredits},#{isRework},#{failReason},#{reworkSituation},unix_timestamp(),unix_timestamp())")
    @Options(useGeneratedKeys=true, keyProperty="id")
    public int addScore(ScoreDO scoreDO);

    @Update("update beihua.score set course_id = #{courseId}, student_id = #{studentId}, grade = #{grade}, grade_point = #{gradePoint}," +
